...
# # Create user and add sudo support
...
# # Install Eigen 3.3.9
# RUN apt-get update -y && apt-get install -y --allow-unauthenticated curl
...
# RUN chown -R user:sharedgroup /home/user/
# RUN chmod -R g+rw /home/user/
# RUN find /home/user/ -type d -exec chmod g+s {} +
...
#     && cd ~/humble_ws && rosdep install -i --from-path src --rosdistro humble -y --skip-keys="ros-humble-joint-state-broadcaster"
# # Suppress the XDG errors when running GUI apps like RVIZ
# RUN mkdir /tmp/${UID}
# RUN chown -R user:sharedgroup /tmp/${UID}
# RUN chmod -R g+rw /tmp/${UID}
# RUN find /tmp/${UID} -type d -exec chmod g+s {} +
...
# # Ensure the user has permissions on the mounted directory
# RUN sudo chown -R user:sharedgroup /home/user/humble_ws/src
# RUN sudo chmod -R g+rw /home/user/humble_ws/src
# RUN sudo find /home/user/humble_ws/src -type d -exec chmod g+s {} +
