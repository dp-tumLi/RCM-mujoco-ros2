...
CallbackReturn FrankaMujocoMultiHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) {
  ...
  for(size_t i = 1; i <= robot_count_; i++){
    ...
    if (info_.hardware_parameters.find("hand" + suffix) != info_.hardware_parameters.end()) {
      has_gripper = info_.hardware_parameters.at("hand" + suffix) == "True";
    }
    ...
    if (arm.robot_->has_gripper_) {
      gripper_states_ptrs_[arm.robot_name_] = std::make_shared<std::array<double, 3>>();
      gripper_nodes_[arm.robot_name_] = std::make_shared<franka_gripper::GripperSimActionServer>(
      ...
      RCLCPP_INFO(getLogger(), "Gripper is not enabled for %s.", arm.robot_name_.c_str());
    }
...
hardware_interface::return_type FrankaMujocoMultiHardwareInterface::read(const rclcpp::Time& /*time*/,
  ...
  for(auto& arm_container_pair: arms_){
    ...
    if (arm.robot_->has_gripper_ && gripper_states_ptrs_.find(arm.robot_name_) != gripper_states_ptrs_.end()) {
